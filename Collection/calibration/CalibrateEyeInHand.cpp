...
vector<string> positionVec = {
	//相机标定坐标
	"A-2505-5509+2117+176+002+172BC",
	"A-3333-6247+2233+169-003+170BC",
	"A-2893-5593+2602+173+005+179BC",
	"A-1527-6140+2212+173-012+164BC",
	"A-1679-5736+1940+169-020-175BC",
	"A-3093-5876+1566+179-000-179BC"
	//热像仪标定坐标
...
void str2Mat(const string strPosition, cv::Mat& toolPose) {
	toolPose.at<double>(0, 0) = std::atoi(strPosition.substr(1, 5).c_str()) / 10.0;
	toolPose.at<double>(0, 1) = std::atoi(strPosition.substr(6, 5).c_str()) / 10.0;
	toolPose.at<double>(0, 2) = std::atoi(strPosition.substr(11, 5).c_str()) / 10.0;
	toolPose.at<double>(0, 3) = std::atoi(strPosition.substr(16, 4).c_str());
	toolPose.at<double>(0, 4) = std::atoi(strPosition.substr(20, 4).c_str());
	toolPose.at<double>(0, 5) = std::atoi(strPosition.substr(24, 4).c_str());
	//cout << "toolPose" << endl << toolPose << endl << endl;
}
...
bool isRotationMatrix(const cv::Mat& R)
{
	...
	shouldBeIdentity = tmp33 * tmp33.t();

	cv::Mat I = cv::Mat::eye(3, 3, shouldBeIdentity.type());
	//cout << "error:" << cv::norm(I, shouldBeIdentity) << endl;
...
void m_calibration(string calibraDataName, vector<string>& FilesName, Size board_size, Size square_size, Mat& cameraMatrix, Mat& distCoeffs, vector<Mat>& rvecsMat, vector<Mat>& tvecsMat)
{
	...
	vector<cv::Point2f> image_points;                                   // 缓存每幅图像上检测到的角点
	...
	for (int i = 0; i < FilesName.size(); i++) {
		...
		cv::Mat originImage;
		cv::Mat originImageDup = cv::imread(FilesName[i]);
		// 左右镜像 flipCode>1：左右
		flip(originImageDup, originImage, 1);
		cv::Mat imageInput;
		cv::cvtColor(originImage, imageInput, cv::COLOR_BGR2GRAY);
		//读入第一张图片时获取图像宽高信息
		if (image_count == 1) {
	...
	for (int i = 0; i < image_count; i++)
	{
		...
		err = norm(cv::Mat(image_points_seq[i]), cv::Mat(image_points_pro), NORM_L2);
...
void cameraCaliration(string calibraDataName, vector<cv::Mat> &tvecsMat, vector<Mat> &rvecsMat)
{
	string file_Directory = calibraDataName + "\\*.bmp";
	vector<cv::String> cvfilenames;
	// cv::String pattem：搜索路径
	//std::vector<cv::String> &result： 存储所有图片绝对路径
...
void CalibrateEyeInHand(bool rgbdOrThermal, string calibraDataName)
{
	//定义手眼标定矩阵
	...
	// 读取末端，标定板的姿态矩阵 4*4
	std::vector<cv::Mat> vecHg, vecHc_camera;
#if rgbdOrThermal
	...
	//************************************************************************************************************
	//标定棋盘格
	cameraCaliration(calibraDataName, tvecsMat, rvecsMat);
	int num_images = tvecsMat.size();

	cv::Mat tempR, tempT;

	//计算标定板位姿 x y z rx ry rz
	...
	//计算机械臂位姿
	cv::Mat ToolPose(num_images, 6, CV_64F, 0.0);
	...
	cv::Mat R_Camera2gripper = R_cam2gripper.clone();
	cv::Mat t_Camera2gripper = t_cam2gripper.clone();
#if rgbdOrThermal
	Hcg_camera = R_T2RT(R_Camera2gripper, t_Camera2gripper);//矩阵合并
#else
	Hcg_thermal = R_T2RT(R_Camera2gripper, t_Camera2gripper);//矩阵合并
#endif
	//************************************************************************************************************
...
#else
	std::cout << "Hcg_thermal 矩阵为： " << std::endl;
	std::cout << Hcg_thermal << std::endl;
#endif


	//ofstream fout(calibraDataName + "\\calibrateHandEye.txt");                       // 保存标定结果的文件
	ofstream fout(calibraDataName + "\\calibrateHandEye.txt", ios::app | ios::out);    // 保存标定结果的文件

#if rgbdOrThermal
	fout << "Hcg_camera 矩阵为： " << endl;
	fout << Hcg_camera << endl;
#else
	fout << "Hcg_thermal 矩阵为： " << std::endl;
	fout << Hcg_thermal << std::endl;
#endif

	cout << "----手眼系统测试----" << endl;
	cout << "机械臂下标定板的空间位置为：" << endl;
...
#else
		cv::Mat worldPos = vecHg[i] * Hcg_thermal * vecHc_camera[i] * cheesePos;
#endif
		fout << i << ": " << worldPos.t() << endl;
	}
	fout.close();
